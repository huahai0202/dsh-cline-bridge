...
import { upstreamSummary } from './upstream.js'
...
export function buildStatus(deps, options) {
  ...
  const models = [...modelSeen.entries()]
    ...
    // 实际上游徽标：只对观测名单里的模型带上（没观测到就是 undefined，面板不显示徽标）。
    // `preferred` 只给「不是预期上游」加视觉提示——插件不干预路由，这个字段不代表任何干预。
    .map(([id, lastUsedAt]) => ({
      id,
      lastUsedAt,
      upstream: upstreamSummary(deps.upstreamLog?.get?.(id), PREFERRED_UPSTREAM[id], lastUsedAt),
    }))
