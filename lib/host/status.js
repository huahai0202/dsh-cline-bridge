 ...
 * 模型名是否像是一个真实的模型 id。
 *
 * 观察到的模型来自「请求体里的 model 字段」与冷却记录，两者都可能被写坏：模型名取自
 * 上游请求体，一旦有请求带了奇怪的 model（手改配置、探针、错误回显），它就会变成一个
 * 筛选芯片，还会经 bumpModelStat 落进状态文件。这里做最低限度的形状校验：
 *   · 非空字符串、长度合理、不含控制字符与换行（避免撑坏面板布局）；
 *   · 至少含一个字母或数字（挡住 '???' / '---' / 空串这类）；
 *   · 排除 readModelOf 用来表示「读不出模型」的 '*'。
 * 刻意**不**做更严格的格式校验：模型 id 的形状由各家提供方自定义，猜格式只会误伤。
 */
...
export function buildStatus(deps, options) {
  ...
  // 面板选定的「使用中」Key：这条通道上只有它的 8 位标签（既不是原文，也不是掩码）。
  // 选定的 key 已不在池中时 selected() 返回 undefined，于是载荷里是 null。
  ...
  return {
    ...
    keys,
    // 面板选定的「使用中」Key（首发送优先用它）：只有 8 位标签，没有原文也没有掩码；
    // 没有选定（或选定的 key 已不在池中）时是 null，面板据此提示「已不在池中」。
    selection: selected ? { label: selected.label } : null,
