...
import { createSettingsReader } from './settings-table.js'
...
export function buildStatus(deps, options) {
  ...
  // 新旧两代设置服务 API 的兼容读取在 settings-table.js：一次构建新建一个 reader，
  // 两个命名空间（llm-pi-ai / agent-default-model）共用同一趟 describe()——新版
  // SettingsForms 的 describe() 每趟遍历 profile 全部条目，不该一构建跑两趟。
  const readSettings = createSettingsReader(deps.settings)
