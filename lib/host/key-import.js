 ...
 * @param options.unknownRefs - 可选。状态**不明**的 ref（凭据服务解析失败、文件也读不到）：
 *   「不知道有没有被占用」不等于「空闲」——这些槽位本次导入一律不用，
 *   否则一次服务抖动就会把用户正在用的 Key 覆盖掉。
...
export async function importClineKeys(options) {
  ...
  const unknownRefs = new Set(options.unknownRefs ?? [])
  ...
  const free = refs.filter((ref) => {
    if (unknownRefs.has(ref)) return false
  ...
  return {
    ...
    // 状态不明而被跳过的槽位（只含本次名单内的 ref），面板据此提示「为什么没往里写」
    refsUnknown: refs.filter((ref) => unknownRefs.has(ref)),
  }
}
