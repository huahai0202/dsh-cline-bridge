...
const TOKEN_FIELDS = ['input', 'output', 'total', 'cached', 'ms'] // ms = 流式耗时累计（毫秒），速度列的分母
...
export function createQuotaStore(path, options) {
  ...
  const write = () => {
    try {
      ...
      writeFileSync(tmp, JSON.stringify({ version: STATE_VERSION, updatedAt: Date.now(), entries, usage: Object.fromEntries(usage), selection: Object.fromEntries(selection), totals, diagnostics }))
      renameSync(tmp, path)
