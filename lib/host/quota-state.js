...
import { MAX_STORED_ERROR_BODY } from './defaults.js'
...
const TOKEN_FIELDS = ['input', 'output', 'total', 'cached', 'ms']
// 上游观测最多记多少条（key+模型 两维；8 把 key × 2 个受盯模型远到不了，只防模型名被乱灌）
const UPSTREAM_MAX_ENTRIES = 400 // ms = 流式耗时累计（毫秒），速度列的分母
/** 面板选定「使用中」Key 的标签形状：与 keyLabel 的输出一致（8 位小写十六进制）。
 *  形状之外的一律丢弃/拒绝——磁盘上的字段只信自己写过的形状。 */
const SELECTION_LABEL_RE = /^[0-9a-f]{8}$/
/** 最多为多少个模型保留选定（面板上真去点过的模型不会多；超了就丢最久以前选的那批）。 */
const SELECTION_MAX_MODELS = 20
...
const DIAG_STRING_FIELDS = ['pluginVersion', 'lastDecision', 'lastExtrasAt', 'pinnedUpstream']
const DIAG_BOOLEAN_FIELDS = ['credentialsFileRead', 'extrasResolved']
const DIAG_NUMBER_FIELDS = ['poolSize', 'clineRequests', 'rotations', 'failFasts', 'statsSince', 'pinApplied']
...
export function createQuotaStore(path, options) {
  ...
  // 每个模型各自的「使用中」Key（首发送优先用它）：model → { label, at }。
  // 冷却与用量本来就是 key+模型 维度，选定同理——给 deepseek 选了某把 key，
  // 不该连带影响 glm。只存 8 位哈希标签与选定时刻。
  let selection = new Map()
  // 上游观测（v2.9.3）：`label\u0000model` → { provider, fallbacks, servedModel, at }。
  // 随状态文件落盘，跨重启保留（用户要求「上游渠道」重启后仍在）；key 只用 8 位标签。
  ...
  /** 选定记录按「最近选过的」裁剪：每个模型一条，长跑进程里模型名可能被不断带进来
   *  （换模型、写错模型的请求），不裁剪就会一直涨。 */
  const trimSelection = () => {
    if (selection.size <= SELECTION_MAX_MODELS) return
    const kept = [...selection.entries()].sort((a, b) => (b[1].at || 0) - (a[1].at || 0))
    selection = new Map(kept.slice(0, SELECTION_MAX_MODELS))
  }
  ...
  try {
    ...
    for (const [label, models] of Object.entries(parsed?.entries ?? {})) {
      ...
      for (const [model, record] of Object.entries(models ?? {})) {
        if (typeof record?.readyAt === 'number' && record.readyAt > now) {
          // 旧版本写下的 body 没有上限，读进来时补夹一刀（回放只需要错误码与恢复窗口）
          const body = typeof record.body === 'string' ? record.body.slice(0, MAX_STORED_ERROR_BODY) : ''
          kept.set(model, { readyAt: record.readyAt, body })
        }
      }
    ...
    // 每个模型各自的「使用中」Key：模型名要像模型、标签要是 keyLabel 的输出，其余一律丢弃。
    // （v2.4.0 曾把它存成单个 { label, at }，字段形状对不上，这里自然读不出来——
    //   那份「全局选定」就此作废，用户按模型重新点一下即可。）
    for (const [model, record] of Object.entries(parsed?.selection ?? {})) {
      if (!isPlausibleModelId(model)) continue
      if (!SELECTION_LABEL_RE.test(String(record?.label ?? ''))) continue
      selection.set(model, { label: record.label, at: count(record.at) })
    }
    trimSelection()
    for (const [key, value] of Object.entries(parsed?.upstream ?? {})) {
      const provider = typeof value?.provider === 'string' ? value.provider.trim() : ''
      if (!provider) continue
      if (!isPlausibleModelId(key) && !key.includes('\u0000')) continue
      upstream.set(key, { provider, fallbacks: count(value.fallbacks), servedModel: typeof value.servedModel === 'string' ? value.servedModel : '', at: count(value.at) })
    }
    if (upstream.size > UPSTREAM_MAX_ENTRIES) {
      upstream = new Map([...upstream.entries()].slice(0, UPSTREAM_MAX_ENTRIES))
    }
  } catch (error) {
  ...
  const write = () => {
    try {
      ...
      // 临时文件名带 pid+时间戳：同 home 目录跑多个实例（或插件被挂两次）时不会互相覆盖对方的 .tmp
      const tmp = path + '.tmp-' + process.pid + '-' + Date.now()
      writeFileSync(tmp, JSON.stringify({ version: STATE_VERSION, updatedAt: Date.now(), entries, usage: Object.fromEntries(usage), selection: Object.fromEntries(selection), upstream: Object.fromEntries(upstream), totals, diagnostics }))
      renameSync(tmp, path)
  ...
  return {
    ...
    /** 某个模型上选定的「使用中」Key（首发送优先用它）；该模型没有选定返回 undefined。 */
    selectionFor(model) {
      const record = selection.get(model)
      return record ? { ...record } : undefined
    },
    /** 全部选定，拍平成面板载荷要的形状：model → 8 位标签（没有选定就是空对象）。 */
    selections() {
      return Object.fromEntries([...selection.entries()].map(([model, record]) => [model, record.label]))
    },
    /** 上游观测的恢复 / 落盘（面板「上游渠道」列跨重启保留）。 */
    upstreamAll() {
      return Object.fromEntries(upstream)
    },
    setUpstream(key, value) {
      const provider = typeof value?.provider === 'string' ? value.provider.trim() : ''
      if (!key || !provider) return false
      upstream.set(key, { provider, fallbacks: count(value.fallbacks), servedModel: typeof value.servedModel === 'string' ? value.servedModel : '', at: count(value.at) })
      if (upstream.size > UPSTREAM_MAX_ENTRIES) upstream.delete(upstream.keys().next().value)
      schedule()
      return true
    },
    /** 覆盖写入某个模型的选定：空值＝取消该模型的选定（幂等）；模型名或标签形状不对时
     *  返回 false 且不改动任何状态。 */
    setSelection(model, label) {
      if (!isPlausibleModelId(model)) return false
      const value = typeof label === 'string' ? label.trim() : ''
      if (!value) {
        if (selection.delete(model)) schedule()
        return true
      }
      if (!SELECTION_LABEL_RE.test(value)) return false
      selection.set(model, { label: value, at: Date.now() })
      trimSelection()
      schedule()
      return true
    },
