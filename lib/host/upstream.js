 ...
 *
 * 落盘（v2.9.3，用户要求）：观测随额度状态文件持久化，跨重启保留。每条观测带 at 时间戳，
 * 面板/排查者可以用它判断新鲜度——持久化的是「X 时刻观察到的是哪家」这个**事实**，
 * 而不是把它伪装成「现在」的结论；旧不旧由看的人拿 at 与当前时间比较。key 仍用
 * 8 位标签，不落原文。
 */
export function createUpstreamLog(initial) {
  ...
  // 重启恢复：把磁盘上的观测灌回内存（形状不对的条目直接丢弃，绝不猜）
  if (initial && typeof initial === 'object') {
    for (const [key, value] of Object.entries(initial)) {
      ...
      if (key.includes('\u0000')) {
        ...
        if (label && model) byKey.set(`${label}\u0000${model}`, entry)
      } else if (isPlausibleModelId(key)) {
  ...
  return {
    ...
    /** 拍平导出（落盘用）：`label\u0000model` → 观测；model 维度不带 key 的也导出。 */
    dump() {
      const out = {}
      for (const [model, entry] of byModel) out[model] = entry
      for (const [key, entry] of byKey) out[key] = entry
      return out
    },
  }
}
