...
/**
 * 最低限度的本机浏览器信任校验。接受两种请求形状：
 *
 *   1) 网页半边（`dsh web`，页面在 http://127.0.0.1:port）：Referer 必须与 Host 同源。
 *   2) 桌面版外壳（Electron，页面在自定义协议 `dsh-app://app` 下）：Chromium 对自定义协议
 *      页面的请求**不发送 Referer**，外壳（`forwardWebRequest`）转发时还会删掉 `origin`
 *      与 `sec-fetch-site`，只留 Host 的会话 cookie 与 Electron 版 UA。此时改用
 *      「无 Referer、无 Origin、非跨站 fetch 元数据、UA 是 Electron」四项一起判定。
 *
 * 为什么第 2 条对网页仍然安全：浏览器发起的跨站请求必然带 `Origin`（CORS 模式）或
 * `sec-fetch-site: cross-site`，而 `no-referrer` 的跨站页面又伪造不了 UA；本地非浏览器
 * 进程本来就能直接读凭据文件，不在本校验的威胁模型里。
 */
export function isTrustedRequest(req) {
  ...
  if (referer !== '') {
    try {
      ...
      // 桌面版外壳：某些 Chromium 版本会把自定义协议的页面 URL 当 Referer 发出来
      if (url.protocol === 'dsh-app:' && url.hostname === 'app') return true
    } catch {
      return false
    }
  }
  // 桌面版外壳：无 Referer / 无 Origin，且请求确实来自 Electron 渲染进程
  return referer === '' && origin === '' && (site === '' || site === 'same-origin') && /\bElectron\//.test(userAgent)
}
