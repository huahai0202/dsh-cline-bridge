    ...
    describe(options) {
      ...
      return [...entries.values()]
        ...
        .map((entry, index) => ({
          ...
          // 有上游观测、但还没有用量行的模型也要出现（重启恢复后这很常见：观测是上次
          // 会话记的，用量行还没在新进程里产生）——否则「上游渠道」列会白白丢一条已知事实。
          models: Object.fromEntries(
            [...new Set([
              ...Object.keys(entry.stats.byModel),
              ...Object.keys(options?.upstreamLog?.dump?.() ?? {}).flatMap((key) => key.includes('\u0000') && key.split('\u0000')[0] === entry.label ? [key.split('\u0000')[1]] : []),
            ])].map((model) => {
              const row = entry.stats.byModel[model] ?? { sent: 0, ok: 0, failed: 0, limited: 0, lastUsedAt: 0, tokens: { input: 0, output: 0, total: 0, cached: 0, ms: 0 } }
              return [model, row]
            }).map(([model, row]) => [
              model,
              {
