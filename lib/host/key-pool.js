...
export function createKeyPool(quotaStore, diag, log) {
  ...
  return {
    ...
    /** 面板选定的「使用中」Key（首发送优先用它）：按标签在池里找回这把 key 原文。
     *  没有选定、或选定的 key 已不在池中（凭据被删 / ref 改过）时返回 undefined——
     *  调用方据此静默退回「首发送用请求自带的 key」。 */
    selected() {
      const label = quotaStore.selection()?.label
      if (!label) return undefined
      const entry = entries.get(label)
      return entry ? { key: entry.key, label: entry.label } : undefined
    },
    /** 面板设定「使用中」的 Key：只接受池内已知的标签（否则返回 false 且不写入任何状态），
     *  空值＝取消选定（幂等）。落盘与跨重启恢复由 quotaStore 负责（只有 8 位标签，无原文）。 */
    setSelection(label) {
      const value = typeof label === 'string' ? label.trim() : ''
      if (!value) {
        quotaStore.setSelection('')
        return true
      }
      if (!entries.has(value)) return false
      return quotaStore.setSelection(value)
    },
