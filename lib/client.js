  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && !document.getElementById(STYLE_ID)) {
      ...
      styleEl.textContent = [
        ...
        '._dsh_ofb_usage_row { display: flex; align-items: baseline; gap: 10px; padding: 6px 14px 6px 22px; font-size: 12px; line-height: 18px; border-bottom: .5px solid var(--dsw-alias-border-l2); }',
        '._dsh_ofb_usage_row:last-child { border-bottom: none; }',
        '._dsh_ofb_usage_model { flex: 1 1 auto; min-width: 0; }',
        '._dsh_ofb_usage_nums { flex: none; width: 104px; text-align: right; font-variant-numeric: tabular-nums; }',
        '._dsh_ofb_usage_time { flex: none; width: 88px; text-align: right; color: var(--dsw-alias-label-tertiary, inherit); }',
    ...
    /** 按模型用量明细：每把 key 逐行列出它在每个模型上真的发了多少次。
     *  这张卡单独存在（而不是塞进表格的用量列）是因为模型名太长——表格里那列只有百来像素，
     *  塞不下「模型名 + 三个计数」；这里整卡宽度可用，模型名可省略号收口并带 title。
     *  跟随模型筛选：选中某个模型时只列那个模型的行。 */
    function UsageByModel(props) {
      ...
      keys.forEach(function (key) {
        ...
        var entries = Object.keys(models)
        ...
        rows.push({ key: key, entries: entries })
      })
      var total = rows.reduce(function (sum, row) { return sum + row.entries.length }, 0)
      return createElement('div', null,
        createElement('h3', { className: '_dsh_ofb_section_title' }, t('usageTitle')),
        createElement('div', { className: '_dsh_ofb_card' },
          total === 0
            ? createElement('div', { className: '_dsh_ofb_empty' }, t('usageEmpty'))
            : createElement('div', { className: '_dsh_ofb_usage' },
                createElement('div', { className: '_dsh_ofb_usage_head' },
                  createElement('span', { className: '_dsh_ofb_usage_model' }, t('usageHint')),
                  createElement('span', { className: '_dsh_ofb_usage_nums' }, t('usageHeadReq')),
                  createElement('span', { className: '_dsh_ofb_usage_nums' }, t('usageHeadTokens')),
                  createElement('span', { className: '_dsh_ofb_usage_time' }, t('colLastUsed')),
                ),
                rows.map(function (row) {
                  return [
                    createElement('div', { key: 'k' + row.key.label, className: '_dsh_ofb_usage_key' },
                      createElement('span', { className: '_dsh_ofb_dim' }, String(row.key.index)),
                      createElement('span', { className: '_dsh_ofb_mono' }, row.key.preview || t('none')),
                      createElement('span', { className: '_dsh_ofb_dim _dsh_ofb_mono' }, row.key.label),
                    ),
                    row.entries.length === 0
                      ? createElement('div', { key: 'e' + row.key.label, className: '_dsh_ofb_usage_row' },
                          createElement('span', { className: '_dsh_ofb_usage_model _dsh_ofb_dim' }, scoped ? t('usageNone') : t('usageEmpty')),
                        )
                      : row.entries.map(function (model) {
                          var stat = row.key.models[model] || {}
                          return createElement('div', { key: model, className: '_dsh_ofb_usage_row' },
                            createElement('span', { className: '_dsh_ofb_clip _dsh_ofb_mono _dsh_ofb_usage_model', title: model }, model),
                            createElement('span', { className: '_dsh_ofb_usage_nums' },
                              String(stat.sent || 0) + '/' + String(stat.ok || 0) + '/' + String(stat.limited || 0)),
                            createElement('span', { className: '_dsh_ofb_usage_nums', title: 'input/output tokens' },
                              tokenPair(stat.tokens)),
