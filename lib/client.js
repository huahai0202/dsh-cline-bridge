...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function UsageByModel(props) {
      ...
      // 条长就该对应输入规模。
      var maxInput = 1
      var sum = { sent: 0, ok: 0, failed: 0, limited: 0, input: 0, output: 0, ms: 0 }
      blocks.forEach(function (block) {
        block.models.forEach(function (model) {
          ...
          sum.ms += Number(tokens.ms) || 0
        })
      })
      // 合计行的速度与每行同口径（Σoutput / Σms 的加权平均）；无耗时数据时显示「—」
      var sumTps = tpsOf({ output: sum.output, ms: sum.ms })
      ...
      var title = createElement('div', { className: '_dsh_ofb_head', style: { minHeight: 0 } },
        ...
        blocks.length
          ? createElement('span', { className: '_dsh_ofb_caption _dsh_ofb_usage_total' },
              t('usageTotal') + ' ' + sum.sent + '/' + sum.ok + '/' + sum.failed + '/' + sum.limited + ' · ' + formatTokens(sum.input) + '/' + formatTokens(sum.output) + ' · ' + t('usageColSpeed') + ' ' + (sumTps === null ? t('tpsNone') : sumTps.toFixed(1)))
          : null,
      )
