  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && !document.getElementById(STYLE_ID)) {
      ...
      styleEl.textContent = [
        ...
        // 固定表格布局 + 每列定宽 + 单元格裁剪：列宽不再由内容撑开，
        // 长内容一律省略号收口（完整值走 title），所以行高恒定、永不横向滚动。
        '._dsh_ofb_table { width: 100%; min-width: 560px; table-layout: fixed; border-collapse: collapse; font-size: 12.5px; line-height: 18px; }',
        '._dsh_ofb_table th { text-align: left; font-weight: 500; font-size: 12px; line-height: 16px; color: var(--dsw-alias-label-tertiary, inherit); background: var(--dsw-alias-bg-layer-3); padding: 9px 10px; border-bottom: .5px solid var(--dsw-alias-border-l2); white-space: nowrap; }',
    ...
    /** 表格列宽（固定布局）：窄列定宽，冷却列吃掉剩余宽度。
     *  定宽 + 裁剪的组合保证长模型名不会把行撑高，也不会把表格撑出横向滚动。
     *  宽度是按最坏情况留的：序号要放得下两位数（池最多 10 把）、状态要放得下「冷却中」药丸、
     *  用量要放得下三位数计数（`999/999/99`）。 */
    var COLUMN_WIDTHS = ['40px', '148px', '78px', 'auto', '96px', '124px']
      ...
      var body = createElement('tbody', null,
        keys.map(function (key) {
          ...
          return createElement('tr', { key: key.label || String(key.index) },
            ...
            createElement('td', null,
              createElement('span', { className: '_dsh_ofb_clip' }, agoOf(lastUsedAt)),
              // 最近用过的模型：筛选到具体模型时不再重复显示它；「全部」时显示短名，完整值放 title
              !scoped && stats.lastModel
                ? createElement('span', { className: '_dsh_ofb_clip _dsh_ofb_mono _dsh_ofb_dim _dsh_ofb_model_small', title: stats.lastModel }, shortModel(stats.lastModel))
                : null,
            ),
          )
        }),
