  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined') {
      ...
      styleEl.textContent = [
        ...
        // 芯片 + 锁定徽标的成组容器：让徽标跟着自己的芯片一起换行，不跟别的芯片错位
        '._dsh_ofb_chipwrap { display: inline-flex; align-items: center; gap: 4px; max-width: 100%; }',
        ...
        // 实际上游徽标：一眼看出「这个模型这次实际走哪家」。
        // 两种状态刻意长得不一样：预期上游=中性（这只是事实陈述）；不是预期上游=警示色
        // （用户真正关心的是别漂走）。注意这里没有「已锁定」这类字样——插件不干预路由，
        // 写「已锁定」会是假话（实测该路由接受 providerOptions 却完全忽略它）。
        '._dsh_ofb_pin { display: inline-flex; align-items: center; gap: 3px; flex: none; height: 18px; padding: 0 7px; border-radius: 999px; font-size: 11px; line-height: 16px; white-space: nowrap; }',
        '._dsh_ofb_pin_ok { border: .5px solid var(--dsw-alias-border-l4); color: var(--dsw-alias-label-tertiary); }',
        '._dsh_ofb_pin_off { border: .5px solid color-mix(in srgb, var(--dsw-alias-state-error-primary, #d4380d) 55%, transparent); background: color-mix(in srgb, var(--dsw-alias-state-error-primary, #d4380d) 10%, transparent); color: var(--dsw-alias-state-error-primary, #d4380d); font-weight: 500; }',
    ...
    var zhDict = {
      ...
      filterLabel: '按模型查看',
      upHere: '上游 {provider}',
      upOther: '上游已变为 {provider}',
      upTitleHere: '这次请求实际由上游 {provider} 服务（另有 {n} 家可兜底）。这是网关汇报的事实，本插件不干预路由。',
      upTitleOther: '注意：这次实际由 {provider} 服务，不是预期的上游（另有 {n} 家可兜底）。上游会随网关打分漂移，且客户端无法锁定。',
      upStale: '这是上一次观察到的结果，本次还没有新数据',
    ...
    var enDict = {
      ...
      upHere: 'via {provider}',
      upOther: 'now via {provider}',
      upTitleHere: 'This request was actually served by upstream {provider} ({n} others available as fallback). Reported by the gateway; this plugin does not influence routing.',
      upTitleOther: 'Note: served by {provider}, not the expected upstream ({n} others available as fallback). The gateway drifts between upstreams and clients cannot pin it.',
      upStale: 'last observed result; no fresh data for the latest request',
    ...
    /** 实际上游徽标：这个模型**这次实际是哪个上游**服务的。
     *
     *  只说实际上游，不说「已锁定」——插件没有能力影响路由（实测该路由会接受
     *  providerOptions.gateway 却完全忽略它，所以锁定代码已移除）。既然锁不住，
     *  至少让「漂到别家」这件事看得见。
     *
     *  两种视觉状态：预期上游（中性）/ 不是预期上游（警示），后者是用户真正关心的。
     *  没有观测数据时不渲染——「看不到」不该显示成某一家的名字。 */
    function UpstreamBadge(props) {
      var upstream = props.upstream
      if (!upstream || !upstream.provider) return null
      var label = t(upstream.other ? 'upOther' : 'upHere', { provider: upstream.provider })
      var title = t(upstream.other ? 'upTitleOther' : 'upTitleHere', {
        provider: upstream.provider,
        n: upstream.fallbacks,
      })
      if (upstream.stale) title += ' · ' + t('upStale')
      return createElement('span', {
        className: '_dsh_ofb_pin ' + (upstream.other ? '_dsh_ofb_pin_off' : '_dsh_ofb_pin_ok'),
        title: title,
      }, label)
    }
    ...
    function ModelFilter(props) {
      ...
      return createElement('div', { className: '_dsh_ofb_filters' },
        ...
        models.map(function (row) {
          ...
          var badge = UpstreamBadge({ upstream: row.upstream })
          if (!badge) return chip
          return createElement('span', { key: row.id, className: '_dsh_ofb_chipwrap' }, chip, badge)
        }),
      )
    }
    ...
    function KeyTable(props) {
      ...
      // 预期上游（主机端按模型给出）：只给「不是它」的值加警示色——用户关心的就是
      // 「这把有没有漂走」。它不代表插件做过任何干预（插件不干预路由）。
      var preferred = (props.upstream && props.upstream.preferred) || ''
    ...
    function TokenCell(props) {
      ...
      // 悬停提示同样要跟随语言：这里曾经是硬编码的「输入 … / 输出 …」，
      // 英文界面下会突然冒出中文（走词典才不会再漏）。
      var exact = t('tokenExact', { input: input, output: output })
    ...
    function ClineKeysSection() {
      ...
      // 当前模型的上游信息（主机端按模型观测来的）：芯片徽标与表格「上游渠道」列共用。
      // 没观测到就是 undefined —— 芯片不显示徽标、表格每行显示「—」。
      var currentUpstream = (models.find(function (row) { return row.id === activeModel }) || {}).upstream
      ...
      if (state.phase === 'error') {
        ...
        body = [
          createElement(KeyTable, {
            ...
            // 当前模型的上游信息（实际上游 + 预期上游）：用来给「不是预期上游」的行加提示
