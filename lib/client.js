...
//
// 数据来自主机半边（index.js）注册的同源只读路由；导入与重置统计走两条同源写路由
// GET /dsh-cline-bridge/keys，每 5 秒轮询一次（页面不可见时跳过）。
  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined') {
      ...
      styleEl.textContent = [
        ...
        // 哈希标签占满剩余宽度（可省略），数字与时间右对齐定宽，谁也不会把谁顶走。
        ...
        blocks.forEach(function (block) {
          ...
          items.push(
            createElement('div', { key: block.key.label, className: '_dsh_ofb_usage_row' },
              ...
              createElement('span', { className: '_dsh_ofb_clip _dsh_ofb_mono _dsh_ofb_dim _dsh_ofb_usage_lead' }, block.key.label),
              createElement('span', { className: '_dsh_ofb_usage_req' },
