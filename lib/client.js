  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && !document.getElementById(STYLE_ID)) {
      ...
      styleEl.textContent = [
        ...
        '._dsh_ofb_kv { display: grid; grid-template-columns: max-content minmax(0, 1fr); gap: 4px 14px; padding: 12px 14px; font-size: 12.5px; line-height: 18px; }',
        '._dsh_ofb_kv_key { color: var(--dsw-alias-label-tertiary, inherit); white-space: nowrap; }',
        ...
        '._dsh_ofb_note { font-size: 12px; line-height: 18px; color: var(--dsw-alias-label-tertiary, inherit); }',
    ...
    var zhDict = {
      ...
      paramTarget: '匹配目标',
      paramCooldown: '默认冷却',
      paramSkipCooling: '跳过冷却中的主 Key',
      paramFailFast: '全池冷却快速失败',
      paramFailFastMin: '快速失败阈值',
      paramStatuses: '触发轮换的状态码',
      paramRefs: '额外 Key 的 ref',
      paramCredFile: '凭据文件',
      paramQuotaFile: '额度状态文件',
      paramMask: '显示 Key 首尾片段',
      paramSource: '额外 Key 来源',
      sourceRequest: 'DSH 请求头',
      on: '开启',
      off: '关闭',
      credentialsRead: '已读取',
      credentialsMissing: '未读到',
      extrasDone: '已解析',
      extrasPending: '待解析',
      minutes: '分钟',
      maskNote: '为便于辨认，这里只显示 Key 首尾各 4 位（掩码预览）；完整 Key 不会出现在面板、日志或磁盘文件中。',
      maskOffNote: '掩码预览已关闭（maskKeyPreview: false），面板不显示任何 Key 片段。',
      agoSeconds: '{n} 秒前',
      agoMinutes: '{n} 分钟前',
      agoHours: '{n} 小时前',
      resetIn: '{time} 后恢复',
    ...
    var enDict = {
      ...
      paramTarget: 'Match target',
      paramCooldown: 'Default cooldown',
      paramSkipCooling: 'Skip cooling primary',
      paramFailFast: 'Fail fast when all cooling',
      paramFailFastMin: 'Fail-fast threshold',
      paramStatuses: 'Rotate on statuses',
      paramRefs: 'Extra key refs',
      paramCredFile: 'Credentials file',
      paramQuotaFile: 'Quota state file',
      paramMask: 'Show key fragments',
      paramSource: 'Extra key sources',
      sourceRequest: 'DSH request header',
      on: 'on',
      off: 'off',
      credentialsRead: 'read',
      credentialsMissing: 'not found',
      extrasDone: 'resolved',
      extrasPending: 'pending',
      minutes: 'min',
      maskNote: 'Only the first and last 4 characters of each key are shown, so you can tell them apart. The full key never appears in this panel, the logs, or the state file.',
      maskOffNote: 'Key fragments are disabled (maskKeyPreview: false); no part of any key is shown.',
      agoSeconds: '{n}s ago',
      agoMinutes: '{n}m ago',
      agoHours: '{n}h ago',
    ...
    function durationOf(ms) {
      var totalMin = Math.round(Number(ms) / 60000)
      if (!isFinite(totalMin) || totalMin <= 0) return '0' + t('minutes')
      if (totalMin < 60) return totalMin + t('minutes')
      var hours = Math.floor(totalMin / 60)
      var minutes = totalMin % 60
      if (hours < 24) return minutes ? hours + 'h' + pad2(minutes) + 'm' : hours + 'h'
      return Math.floor(hours / 24) + 'd' + (hours % 24) + 'h'
    }

    /** 来源：'request' 是 DSH 自己发的那把，其余（config/env/凭据文件）原样展示。 */
    function sourceOf(key) {
      var source = key && key.source ? String(key.source) : 'unknown'
      return source === 'request' ? t('sourceRequest') : source
    }
    ...
    function KeyTable(props) {
      ...
      var head = createElement('thead', null,
        createElement('tr', null,
          ...
          createElement('th', null, t('colSource')),
      ...
      var body = createElement('tbody', null,
        keys.map(function (key) {
          ...
          return createElement('tr', { key: key.label || String(key.index) },
            ...
            createElement('td', { className: '_dsh_ofb_caption' }, sourceOf(key)),
    ...
    function ParamsCard(props) {
      var data = props.data || {}
      var settings = data.settings || {}
      var extras = data.extras || {}
      var rows = [
        [t('paramTarget'), settings.clineMatch],
        [t('paramCooldown'), durationOf(settings.clineCooldownMs)],
        [t('paramSkipCooling'), settings.skipCoolingRequestKey ? t('on') : t('off')],
        [t('paramFailFast'), settings.allCoolingFailFast ? t('on') : t('off')],
        [t('paramFailFastMin'), durationOf(settings.failFastMinMs)],
        [t('paramStatuses'), (settings.rotateStatuses || []).join(', ')],
        [t('paramMask'), settings.maskKeyPreview ? t('on') : t('off')],
        [t('paramRefs'), Array.isArray(settings.clineKeyRefs) ? settings.clineKeyRefs.join(', ') : ''],
        [t('paramCredFile'), settings.credentialsFile + (extras.credentialsFileRead ? ' (' + t('credentialsRead') + ')' : ' (' + t('credentialsMissing') + ')')],
        [t('paramSource'), extras.extrasResolved ? t('extrasDone') : t('extrasPending')],
        [t('paramQuotaFile'), data.quotaStatePath],
      ]
      return createElement('div', null,
        createElement('h3', { className: '_dsh_ofb_section_title' }, t('paramsTitle')),
        createElement('div', { className: '_dsh_ofb_card' },
          createElement('div', { className: '_dsh_ofb_kv' },
            rows.map(function (row, index) {
              return [
                createElement('span', { key: 'k' + index, className: '_dsh_ofb_kv_key' }, row[0]),
                createElement('span', { key: 'v' + index, className: '_dsh_ofb_mono', style: { overflowWrap: 'anywhere' } }, String(row[1] ?? t('none'))),
              ]
    ...
    function ClineKeysSection() {
      ...
      if (state.phase === 'error') {
        ...
        body = [
          ...
          createElement('div', { key: 'note', className: '_dsh_ofb_note' },
            settings.maskKeyPreview === false ? t('maskOffNote') : t('maskNote'),
          ),
          createElement(RecentList, { key: 'recent', recent: data.recent }),
          createElement(ParamsCard, { key: 'params', data: data }),
        ]
      }
