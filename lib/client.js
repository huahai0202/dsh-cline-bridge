  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined') {
      ...
      styleEl.textContent = [
        ...
        '._dsh_ofb_table { width: 100%; min-width: 620px; table-layout: fixed; border-collapse: collapse; font-size: 12.5px; line-height: 18px; }',
    ...
    var zhDict = {
      ...
      useSet: '使用',
      useActive: '使用中',
      useHint: '选定后首发送优先使用这把 Key；撞限流仍照常换 Key 重发',
      useClearHint: '点击取消选定（首发送回到 DSH 配置的 Key）',
      useMissing: '选定的 Key 已不在池中（首发送回到 DSH 配置的 Key）',
    ...
    var enDict = {
      ...
      useSet: 'Use',
      useActive: 'In use',
      useHint: 'The first send uses this key; rotation on rate limit is unchanged',
      useClearHint: 'Click to clear (the first send returns to the key configured in DSH)',
      useMissing: 'The selected key is no longer in the pool (the first send falls back to the DSH key)',
     ...
     *  容器低于 min-width(620px) 时才退回横向滚动。 */
    var COLUMN_WIDTHS = ['5%', '20%', '12%', '20%', '16%', '15%', '12%']

    /** 行内「使用」按钮：把某把 Key 设为「使用中」（首发送优先用它）。
     *  再点一次已选中的那把＝取消（首发送回到 DSH 配置的 Key）。 */
    function UseButton(props) {
      return createElement('button', {
        ...
        title: props.active ? t('useClearHint') : t('useHint'),
    ...
    function ClineKeysSection() {
      ...
      /** 选定 / 取消「使用中」的 Key（首发送优先用它）。写路由成功后重拉一次：
       *  徽标、状态与统计一起刷新，不必等下一次 5 秒轮询。 */
      async function doSelect(label, active) {
        ...
        try {
          var res = await fetch(SELECT_ROUTE, {
            ...
            headers: { 'content-type': 'application/json', accept: 'application/json' },
            cache: 'no-store',
            // 点已选中的那把＝取消：空标签是主机端约定的「清除」写法
            body: JSON.stringify({ label: active ? '' : label }),
          })
          var payload = null
          try { payload = await res.json() } catch (parseError) { payload = null }
          if (!res.ok) throw new Error((payload && payload.error) || ('HTTP ' + res.status))
      ...
      // 选定的「使用中」Key：载荷里只有标签。它已不在池中（凭据被删）时单独给一行提示——
      // 否则用户只会看到徽标莫名消失，却不知道首发送已经回到 DSH 配置的 Key。
      var selectionLabel = (data && data.selection && data.selection.label) || ''
