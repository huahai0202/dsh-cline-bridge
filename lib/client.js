  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined') {
      ...
      styleEl.textContent = [
        ...
        // 统计区：4 列网格（7 张数据卡 + 导入按钮正好两行排满，任何宽度下都对齐，
// 不会出现截图里「导入按钮孤行带一个洞」的观感）
'._dsh_ofb_stats { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 8px; }',
        // 统计小卡：DSH 设置卡片同款表面（layer-3 填充 + 描边高度）
        '._dsh_ofb_stat { display: flex; flex-direction: column; gap: 2px; padding: 8px 12px; border-radius: 12px; background: var(--dsw-alias-bg-layer-3); box-shadow: var(--dsw-elevation-stroke); }',
        '._dsh_ofb_stat_action { justify-content: center; align-items: center; }',
        '._dsh_ofb_stat_value { font-size: 16px; line-height: 22px; font-weight: 600; font-variant-numeric: tabular-nums; }',
        '._dsh_ofb_stat_action > div { display: flex; }',
        '._dsh_ofb_stat_action button { flex: 1 1 auto; justify-content: center; }',
        ...
        // 冷却列：模型单独一行（可省略），倒计时与绝对时刻排成一行。格内放不下时
        // （列宽是百分比，窄容器里 22% 放不下「1h13m 09-21 09:10」一整行）时刻折到
        // 第二行——绝不允许越界压进隔壁「上游渠道」格：td 上的 overflow:hidden 对
        // 表格单元格不生效（table-cell 不是 block container，浏览器会忽略），所以
        // 每格的超宽防护必须由格内自己的裁剪壳完成，这里靠 flex-wrap + 时刻的裁剪类。
        '._dsh_ofb_cooling_line { display: flex; flex-wrap: wrap; align-items: baseline; gap: 2px 6px; min-width: 0; }',
        '._dsh_ofb_remaining { color: var(--dsw-alias-state-error-primary); font-variant-numeric: tabular-nums; flex: none; }',
    ...
    var zhDict = {
      ...
      importHint: '一行一把，也可用空格 / 逗号分隔。写入凭据仓库的空闲槽位（CLINE_API_KEY_2 ~ _12），池内已存在的自动跳过；导入后立刻参与轮换。',
      ...
      tpsExact: '输出速度 {tps} tok/s = 输出 {output} token ÷ 流式 {seconds} 秒',
      tpsNone: '—',
      tpsTitle: '输出速度 = 输出 token ÷ 流式耗时（累计加权平均，tok/s）。换 Key 或换上游后最容易在这里看出快慢；缓存命中不影响这个值，也不影响免费额度。',
      tpsTitleNone: '这个模型上还没有流式请求（或只有非流式），算不出速度（显示「—」而不是 0）。',
      usageTotal: '合计',
    ...
    var enDict = {
      ...
      importHint: 'One key per line (spaces or commas also work). They are stored in the free credential slots (CLINE_API_KEY_2 ~ _12); keys already in the pool are skipped, and new ones rotate immediately.',
      importPlaceholder: 'sk_...',
      ...
      speedConfirm: 'Confirm speed reset',
      ...
      tpsExact: 'Output speed {tps} tok/s = {output} output tokens ÷ {seconds}s streaming',
      tpsNone: '—',
      tpsTitle: 'Output speed = output tokens ÷ streaming time (cumulative weighted average, tok/s). The easiest place to spot a slow key or upstream; cache hits do not affect this value, nor your free quota.',
      tpsTitleNone: 'No streaming request on this model yet, so no speed can be computed (shown as “—” rather than 0).',
      usageTotal: 'Total',
    ...
    function CoolingCell(props) {
      ...
      return createElement('span', { className: '_dsh_ofb_cooling_line' },
        ...
        // 时刻带裁剪壳 + title：极窄时自己省略号收口，完整时刻悬停可见
        createElement('span', { className: '_dsh_ofb_clip _dsh_ofb_dim', title: stamp }, stamp),
      )
    }
    ...
    /** 输出速度：`output / (ms/1000)`（tok/s）。
     *
     *  两个数都是累计值：累计输出 ÷ 累计流式耗时 = 按 token 加权的平均速度，正是想看的量。
     *  ms 为 0（非流式、或该模型上还没跑过）返回 null —— 面板显示「—」，而不是编一个速度。
     *  异常防护：ms>0 但 output=0 时也返回 null（没有产出就谈不上速度）。
     *  缓存命中率不再展示：它不影响免费额度（额度按原始 prompt token 计），也不影响回答
     *  质量，只影响上游给 Cline 的采购成本——对用户是「别人的指标」。速度才是体感。 */
    function tpsOf(tokens) {
      var output = Number((tokens || {}).output) || 0
      var ms = Number((tokens || {}).ms) || 0
      if (output <= 0 || ms <= 0) return null
      return (output * 1000) / ms
    }

    /** Token 单元：占比条 + 「输入 | 速度 tok/s」两个数字。
     *
     *  条的**长度**按同卡内最大输入量归一（「哪把 key 用得多」一眼可比，纯数字做不到）。
     *  第二个数字是这把 key 在该模型上的累计平均输出速度——换上游/换 key 后最容易
     *  被它发现（上游慢了这里立刻掉）。
     *
     *  速度直接从 tokens 算（output 与 ms 都在载荷里，不额外下发派生字段）。 */
    function TokenCacheCell(props) {
      var tokens = props.tokens || {}
      var input = Number(tokens.input) || 0
      var tps = tpsOf(tokens)
      ...
      var tpsTip = tps === null
        ? t('tpsTitleNone')
        : t('tpsExact', { output: Number(tokens.output) || 0, seconds: ((Number(tokens.ms) || 0) / 1000).toFixed(1), tps: tps.toFixed(1) }) + ' · ' + t('tpsTitle')
        ...
        createElement('div', { className: '_dsh_ofb_bar', title: exact },
          input > 0
            ? createElement('div', { className: '_dsh_ofb_bar_fill', style: { width: (scale * 100).toFixed(1) + '%' } },
                createElement('div', { className: '_dsh_ofb_bar_in' }),
              )
            : null,
        ),
        createElement('div', { className: '_dsh_ofb_tokennums' },
          createElement('span', { title: exact }, formatTokens(input)),
          createElement('span', { title: tpsTip }, tps === null ? t('tpsNone') : tps.toFixed(1)),
        ),
      )
    }
    ...
    function UsageByModel(props) {
      ...
      // 条长就该对应输入规模。
      var maxInput = 1
      var sum = { sent: 0, ok: 0, failed: 0, limited: 0, input: 0, output: 0 }
      ...
      // 合计行不再显示速度（用户要求）：跨 key 加权平均没有「哪把快」的信息量，
      // 快慢是每行自己的事，看行内数字即可。
      ...
      var title = createElement('div', { className: '_dsh_ofb_head', style: { minHeight: 0 } },
        ...
        blocks.length
          ? createElement('span', { className: '_dsh_ofb_caption _dsh_ofb_usage_total' },
              t('usageTotal') + ' ' + sum.sent + '/' + sum.ok + '/' + sum.failed + '/' + sum.limited + ' · ' + formatTokens(sum.input) + '/' + formatTokens(sum.output))
          : null,
      )
          ...
          createElement('div', { key: 'head', className: '_dsh_ofb_usage_head' },
            ...
            createElement('span', { className: '_dsh_ofb_tokencell' },
              createElement('span', { className: '_dsh_ofb_tokennums' },
                ...
                createElement('span', { title: t('tpsTitle') }, t('usageColSpeed')),
              ),
            ),
    ...
    function ImportButton(props) {
      // 放进统计网格的最后一格：和统计卡同高同表面，按钮居中——不再是游离在网格外的孤行
      return createElement('div', { className: '_dsh_ofb_stat _dsh_ofb_stat_action' },
    ...
    function ClineKeysSection() {
      ...
      var [speedArmed, setSpeedArmed] = React.useState(false)
      ...
      React.useEffect(function () {
        if (!speedArmed) return undefined
        var timer = setTimeout(function () { setSpeedArmed(false) }, 4000)
        return function () { clearTimeout(timer) }
      }, [speedArmed])
      ...
      /** 只重置速度数据（tokens.ms），计数 / token / 冷却一概不动。两段式确认与
       *  重置统计同款——虽然只丢速度数据，误触仍会让速度列空转几轮才恢复。 */
      async function doResetSpeed() {
        if (resetBusy) return
        if (!speedArmed) {
          setSpeedArmed(true)
          setResetNote('')
        ...
        try {
          var res = await fetch(RESET_ROUTE, {
            ...
            headers: { 'content-type': 'application/json', accept: 'application/json' },
            cache: 'no-store',
            body: JSON.stringify({ scope: 'speed' }),
          })
          var payload = null
          try { payload = await res.json() } catch (parseError) { payload = null }
          if (!res.ok) throw new Error((payload && payload.error) || ('HTTP ' + res.status))
          // 成功时不出提示：数据列自己会变成「—」（用户要求不显示「已重置」文案）；失败仍要提示
          reload()
        } catch (requestError) {
          setResetNote(t('resetFailed', { message: requestError && requestError.message ? requestError.message : String(requestError) }))
        } finally {
      ...
      var header = createElement('div', { className: '_dsh_ofb_head' },
        ...
        createElement('div', { className: '_dsh_ofb_head_actions' },
          ...
          createElement(Button, {
            variant: 'outline',
            onClick: doResetSpeed,
            disabled: resetBusy,
          }, resetBusy ? t('resetBusy') : speedArmed ? t('speedConfirm') : t('speedReset')),
          createElement(Button, {
            variant: 'outline',
            onClick: reload,
