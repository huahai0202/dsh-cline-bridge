  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined') {
      ...
      styleEl.textContent = [
        ...
        // 表头一律不换行：表头被折成两行（「发送/成功/限流」→「发送/成功/限 / 流」）比截断更难看，
        // 所以表头文案保持极短（请求 / Token），完整含义放在 title 里。
        ...
        '._dsh_ofb_usage_nums { text-align: left; font-variant-numeric: tabular-nums; font-size: 11.5px; line-height: 15px; }',
    ...
    var enDict = {
      ...
      usageHeadReq: 'sent / ok / rate-limited',
      ...
      if (blocks.length === 0) {
        ...
        var items = [
          createElement('div', { key: 'head', className: '_dsh_ofb_usage_head' },
            ...
            createElement('span', { className: '_dsh_ofb_usage_req', title: t('usageHeadReq') }, t('usageColReq')),
            // 用与数据行相同的单元格类 → 表头文字与下面的条左端对齐
            createElement('span', { className: '_dsh_ofb_tokencell', title: t('usageHeadTokens') }, t('usageColTokens')),
