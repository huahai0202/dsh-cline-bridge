...
**按模型分别查看。** 面板顶部有一排筛选：`全部模型` + 该通道下的每个模型，**默认选中你当前正在用的那个模型**。这一条是必需的——冷却与用量都是 `key + 模型` 维度：一把 Key 在 `deepseek-v4.1-flash` 上撞了每日上限，在 `glm-5.3-flash` 上往往照常可用；只按「有没有冷却记录」判断，会让人误以为池子已经没 Key 可用。筛选生效后，状态药丸、冷却列、用量列、最近使用**全部只看该模型**，概览卡的「该模型可用 / 该模型冷却」讲的也是这个模型的可用把数；切到 `全部模型` 则回到各模型的汇总与总计。
...
| --- | --- |
| **#** | 池内序号，按最近使用倒序 |
| **Key** | 首尾各 4 位的掩码预览 + 8 位哈希标签；首个发起请求的那把会带「主 Key」标记 |
| **状态** | 可用 / 冷却中（只看当前筛选的模型） |
| **冷却 / 恢复** | 该 Key 在哪些模型上撞了每日上限，以及距恢复的倒计时与恢复时刻；筛到具体模型时只列该模型的记录（模型名不再重复显示） |
| **请求 / Token** | 上行：真的发给上游 / 成功 / 撞限流的次数；下行：**输入/输出 token**。筛到具体模型时都是该模型的数字，`全部模型` 时是总计 |
| **最近使用** | 相对时间；筛到具体模型时是该模型的最近使用（从没跑过就显示「从未」）。用到的模型名放在 `title` 里，悬停可见 |

面板顶部还有池大小、可用/冷却把数、`Cline 请求` / `换 Key 恢复` / `快速失败` 三个累计计数与插件版本；表格下方依次是**「按模型用量」**与「最近决策」（最近几条请求的模型与决策文本）。面板每 5 秒自动刷新一次（页面不可见时暂停），也可手动刷新。

**按模型用量（含 token）。** 每把 Key 逐行列出它在**每个模型**上的实际用量——`发送/成功/限流` 与 **`输入/输出 Token`**，以及最近使用时间：

```
1  sk_l…4444  db694bbf
   z-ai/glm-5.3-flash               3/3/0    4.5k/300     4 秒前
   cline-free/deepseek-v4.1-flash  16/15/1   12.3k/1.2k   1 小时前
2  sk_t…8888  761f9875
   z-ai/glm-5.3-flash              21/21/0   21k/2.1k     9 秒前
```

token 数按 k/M 压缩显示（`12.3k`、`1.23M`）。这张卡单独存在而不是塞进表格的用量列，是因为模型名太长：表格那列只有百来像素，放不下「模型名 + 两组数字」。同样的理由，它也跟随模型筛选——选中某个模型时只列那个模型的行，该模型上没用过的 Key 会明确写「该模型上没用过」。

> **token 从哪来**：插件位于 openai SDK 之下，是唯一能碰到原始响应体的层次，所以用量只能在这里捞：**流式**（Cline 走的就是这条）取 SSE 里最后一个带 `usage` 的 data chunk——pi-ai 已经带上了 `stream_options.include_usage`；**非流式**取 JSON 体里的 `usage`。实现上把响应体 `tee` 成两路，一路原样交给上层 SDK，另一路只挑 `usage` 读一遍即丢，所以响应内容与流式行为完全不变（自检里专门断言了 SSE 与 usage 原样送达）。只有 2xx 才挂这个观察分支，错误/重试路径一概不碰。用量按 `key + 模型` 记录，同时累加到 Key 级总计；**只存在内存里**，落盘的状态文件仍然只写冷却信息。
