...
**按模型分别查看。** 面板顶部有一排筛选：`全部模型` + 每个出现过的模型，**默认选中你当前正在用的那个模型**。这一条是必需的——冷却与用量都是 `key + 模型` 维度：一把 Key 在 `deepseek-v4.1-flash` 上撞了每日上限，在 `glm-5.3-flash` 上往往照常可用；只按「有没有冷却记录」判断，会让人误以为池子已经没 Key 可用。筛选生效后，状态药丸、冷却列、用量列、最近使用**全部只看该模型**，概览卡的「该模型可用 / 该模型冷却」讲的也是这个模型的可用把数；切到 `全部模型` 则回到各模型的汇总与总计。
...
| **状态** | 可用 / 冷却中（只看当前筛选的模型） |
| **冷却模型 / 恢复** | 该 Key 在哪些模型上撞了每日上限，以及距恢复的倒计时与绝对时刻；筛到具体模型时只列该模型的记录 |
| **发送 / 成功 / 限流** | 本次运行（进程内）真的发给上游 / 成功 / 撞限流的次数；筛到具体模型时是该模型的计数，`全部模型` 时是总计 |
| **最近使用** | 相对时间；筛到具体模型时是该模型的最近使用（从没跑过就显示「从未」），`全部模型` 时附带模型短名（完整名悬停可见） |

面板顶部还有池大小、可用/冷却把数、`Cline 请求` / `换 Key 恢复` / `快速失败` 三个累计计数与插件版本；底部是「最近决策」（最近几条请求的模型与决策文本）。面板每 5 秒自动刷新一次（页面不可见时暂停），也可手动刷新。

> 表格用固定列宽 + 省略号收口：长模型名不会把行撑高、也不会把表格撑出横向滚动；被截断的值（完整模型名等）都放在 `title` 里，鼠标悬停可看全。
