...
- **面板直接选定「使用中」的 Key**：表格最后一列每行一个「使用」按钮，点一下它就成了**首发送优先使用**的那把（覆盖 DSH 配置的主 Key；请求没带 key 时也由它补上）。选定只决定「从哪把开始」——**撞限流后照常换 Key 重发**（不是独占模式）；再点一次取消，回到「首发送用 DSH 配置的 Key」。选定只以 8 位标签落在额度状态文件里（不含原文），跨重启保留，不写凭据、不改 DSH 配置；
...
- **面板可以选定「使用中」的 Key**（表格最后一列的「使用」按钮）：选定后**首发送改用这把**（覆盖请求自带的 Key，请求没带 key 时也由它补上），其余一切照旧——撞限流后照样换 Key 重发，所以它不是「只许用这把」的独占模式。选定的 Key 若本地已记冷却，处置与主 Key 完全一致（默认仍先试一次；`skipCoolingRequestKey: true` 时同样会被跳过、改用健康 Key）。选定状态与冷却、统计共用同一个状态文件（**只有 8 位标签**，跨重启保留）；选定的 Key 已不在池中（凭据被删）时静默退回「首发送用请求自带的 Key」，面板会提示一句。再点一次该按钮即取消。
...
| **使用** | 「使用」按钮：点一下把这把设为**首发送优先使用**（语义见「行为约定」）；显示「使用中」的那把再点一次即取消 |

> **「使用」列做什么、不做什么**：选定只决定插件发请求时**从哪把开始**——它覆盖请求自带的 Key，但**不接管轮换**（撞限流后照常换 Key 重发）；也**不改凭据**、不改 DSH 配置，随时可以取消。选定与冷却、统计共用同一个状态文件（只有 8 位标签，跨重启保留）；选定的 Key 已不在池中（凭据被删）时，表格上方会提示「选定的 Key 已不在池中」，此时首发送自动回到 DSH 配置的 Key。
...
> 表格列宽按**百分比**分配（7 列合计 100%），并且**表头与单元格都做省略号收口**：侧栏窄下来时每一列只会在自己的格子里截断，不会几列标题挤成一片，长模型名也不会把行撑高；容器窄于 620px 时才退回横向滚动。被截断的值（完整模型名等）都放在 `title` 里，鼠标悬停可看全。
...
> | `POST /dsh-cline-bridge/keys/select` | **写**：选定 / 取消「使用中」的 Key（首发送优先用它） | 与导入同一套闸门（`keyImport: false` 一并关掉）；正文上限 1KB。只接受**池内已知的 8 位标签**，空标签＝取消（幂等）；回包只有标签与池大小，且在只读载荷里同步可见。不写凭据、不改 DSH 配置 |
>
...
| 文件 | 行数 | 职责 |
| --- | --- | --- |
| `index.js` | ~764 | 插件入口：装配各模块 + 实现 fetch 层拦截（Cline 特征头注入 + 换 Key 轮换）+ 面板只读/导入/重置/选定四条路由 |
| `lib/host/defaults.js` | ~82 | 插件版本、Cline 通道可调常量、DSH 路径解析、旧状态文件迁移 |
| `lib/host/labels.js` | ~47 | key 的 8 位哈希标签与首尾掩码预览 |
| `lib/host/quota-state.js` | ~349 | 状态落盘：额度冷却 + **用量统计 / 累计计数** + 面板选定的「使用中」Key（同一文件、同一去抖写入器）+ 诊断白名单 + 重试窗口解析（报文文本与 `Retry-After` 头） |
...
| `lib/host/key-pool.js` | ~404 | key 池：来源、按 `key+模型` 冷却、粘性选 Key、面板选定的「使用中」key、按模型用量（按 8 位标签跨重启恢复） |
| `lib/host/status.js` | ~178 | 设置面板的状态载荷（唯一的对外数据出口，字段白名单） |
| `lib/host/http.js` | ~107 | 路由的 JSON 响应（含 ETag / 304 条件请求）、同源校验、带上限的 JSON 正文读取 |
| `lib/client.js` | ~1183 | 浏览器半边：设置页分区（含导入按钮与弹窗、重置统计、选定「使用中」的 Key）（**必须单文件**，见下） |
...
`cline-key-check.mjs` 覆盖：换 Key 恢复、按模型冷却、**粘性选 Key（先烧完一把再换下一把）**、三种 Key 来源（config / 环境变量 / 凭据仓库）、单 Key 与瞬时限流下的收尾差异、**冷却窗口只认服务端（报文文本 → `Retry-After` 头，含 HTTP-date、「报文优先」与「拿不到就不记冷却」）**、**额度状态跨重启持久化**、**全池冷却快速失败**、**用量统计跨重启持久化（含粘性延续）**、**面板选定的「使用中」Key（V 组：首发送优先用它、撞限流照常轮换、跨重启保留、落盘只有 8 位标签）**。
...
- **选定「使用中」的 Key（W / X 组）**：写路由的每道闸门（非 `POST` → 405、非同源 → 403、非 JSON → 415、坏 JSON → 400、非字符串标签 → 400）、池内没有的标签被拒且不改动现状、选定后只读载荷同步可见、取消（空标签）是幂等清空、`keyImport: false` 一并关掉、**状态文件里只有 8 位标签**、跨重启保留；浏览器半边则走完整条链路——表格末尾的「使用」列与每行按钮、点击发出一次 JSON POST（body 是这把的 8 位标签）、成功后自动重拉、选定的那把显示「使用中」、再点一次发空标签取消、选定的 Key 不在池中时给出提示。
