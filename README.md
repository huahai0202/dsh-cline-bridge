...
`cline-key-check.mjs` 覆盖：换 Key 恢复、按模型冷却、**粘性选 Key（先烧完一把再换下一把）**、三种 Key 来源（config / 环境变量 / 凭据仓库）、单 Key 与瞬时限流下的收尾差异、**冷却窗口只认服务端（报文文本 → `Retry-After` 头，含 HTTP-date、「报文优先」与「拿不到就不记冷却」）**、**额度状态跨重启持久化**、**全池冷却快速失败**、**用量统计跨重启持久化（含粘性延续）**、**面板选定的「使用中」Key（V 组：按模型独立——给一个模型选定不影响另一个、该模型首发送优先用它、撞限流照常轮换且选定跟着挪到跑通的那把、没选定的模型不会被自动选中、跨重启保留、落盘只有 8 位标签）**。
