...
> 第二张「按模型用量」表**保持不变**（仍是请求计数 / token / 最近使用）。

芯片徽标悬停可看兜底家数，以及「这是网关汇报的事实、插件不干预路由」；观测结果早于该模型最近一次使用时，title 会注明「这是上一次观察到的结果，本次还没有新数据」——**不把旧结论冒充当前状态**。没有任何观测数据时既不渲染徽标、表格里也是「—」。
