...
**按模型用量（含 token 与输出速度）。** 面板始终处在某个模型的筛选下（筛选芯片没有「全部模型」），所以这张卡把每把 Key 在**当前模型**上的实际用量压成**一行**——Key 预览 + `发送/成功/失败/限流` + **`输入/速度 tok/s`**（配一根占比条）+ 最近使用时间；标题行给出当前范围的**合计**（含整体加权速度）。模型名不在这张卡里重复出现——它已经写在筛选芯片上了：
