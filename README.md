...
- **表头用极短标签**（`请求` / `Token`），完整含义（`发送 / 成功 / 撞限流`、`输入 / 输出 token`）放在悬停提示里：长表头在窄栏里会被折成两行（`发送/成功/限` + `流`），比截断更难看。`Token` 表头与下面的条**左端对齐**，数字也左对齐，整格读起来是一条竖线。
...
- 这张卡单独存在而不是塞进表格的用量列，是因为模型名太长：表格那列只有百来像素，放不下「模型名 + 两组数字」；这里整卡宽度可用，模型名能完整显示。同样地，它也跟随模型筛选。
