...
| 文件 | 行数 | 职责 |
| --- | --- | --- |
| `index.js` | ~709 | 插件入口：装配各模块 + 实现 fetch 层拦截（Cline 特征头注入 + 换 Key 轮换）+ 面板只读/导入/重置三条路由 |
| `lib/host/defaults.js` | ~82 | 插件版本、Cline 通道可调常量、DSH 路径解析、旧状态文件迁移 |
| `lib/host/labels.js` | ~47 | key 的 8 位哈希标签与首尾掩码预览 |
| `lib/host/quota-state.js` | ~322 | 状态落盘：额度冷却 + **用量统计 / 累计计数**（同一文件、同一去抖写入器）+ 诊断白名单 + 重试窗口解析（报文文本与 `Retry-After` 头） |
...
| `lib/host/key-pool.js` | ~384 | key 池：来源、按 `key+模型` 冷却、粘性选 Key、按模型用量（按 8 位标签跨重启恢复） |
| `lib/host/status.js` | ~172 | 设置面板的状态载荷（唯一的对外数据出口，字段白名单） |
| `lib/host/http.js` | ~107 | 路由的 JSON 响应（含 ETag / 304 条件请求）、同源校验、带上限的 JSON 正文读取 |
| `lib/client.js` | ~1091 | 浏览器半边：设置页分区（含导入按钮与弹窗、重置统计）（**必须单文件**，见下） |
...
`cline-key-check.mjs` 覆盖：换 Key 恢复、按模型冷却、**粘性选 Key（先烧完一把再换下一把）**、三种 Key 来源（config / 环境变量 / 凭据仓库）、单 Key 与瞬时限流下的收尾差异、**冷却窗口只认服务端（报文文本 → `Retry-After` 头，含 HTTP-date、「报文优先」与「拿不到就不记冷却」）**、**额度状态跨重启持久化**、**全池冷却快速失败**、**用量统计跨重启持久化（含粘性延续）**。
