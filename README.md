...
- **主 Key 挂载即入池。** 插件从 DSH 设置里 Cline 提供方的 `apiKeyEnv` 解析出主 Key（凭据服务优先，缺席时直读凭据文件），挂载时就把它登记进池子——所以重启后面板立刻是全量，不必先发一条请求才变全。来源仍标 `DSH 请求头`（它本来就是 DSH 会携带的那把）；settings 或凭据服务缺席时静默跳过，行为不变。在 DSH 0.1.7+ 上还会订阅设置变更事件（`settings/document-updated`）：你在设置里改了 Cline 提供方的 `apiKeyEnv`，新主 Key **立刻**进池，不必重启。
...
> **模型清单从哪来**：= DSH 设置里 **Cline 通道（baseURL 命中 `clineMatch` 的提供方）配置的模型** ∪ **实际观察到活动的模型**（流量、冷却记录）。只看流量的话，重启后还没发过请求的模型会整个消失（面板里只剩有冷却记录的那个模型，看着像少了模型）；只看配置的话，临时跑过的其它模型又不会出现。默认筛选项的优先级是：最近一次真实流量的模型 → DSH 的 `agent-default-model`（若它也在 Cline 名下）→ 配置里的第一个。`settings` 服务缺席时自动退化为「只列观察到的模型」，不会报错。设置读取同时兼容两代 DSH：旧版的 `settings.get(ns)`（≤0.1.6）与 0.1.7+ 表单式 `SettingsForms.describe()`（见 `lib/host/settings-table.js`）。
...
面板顶部还有池大小、可用/冷却把数、`Cline 请求` / `换 Key 恢复` / `快速失败` 三个累计计数与插件版本；**统计区是 4 列网格**（7 张数据卡 + 「导入 Key」占最后一格，两行排满、任何宽度都对齐）；表格下方依次是**「按模型用量」**与「最近决策」（最近几条请求的模型与决策文本）。面板每 5 秒自动刷新一次（页面不可见时暂停），也可手动刷新。轮询走 **条件请求**：主机端按内容算 `ETag`，载荷没变就回 `304`，省掉整份 JSON 的传输与解析——而绝大多数轮次（冷却倒计时按分钟取整）内容确实没变。
...
顶部右侧的**「重置统计」**与**「重置速度」**按钮都是两段式（先点一下变「确认重置」，4 秒内不点会自动撤回，再点才真的执行）：把计数、token、最近决策与统计起点全部归零并立刻落盘；**冷却与额度不受影响**——那是服务端的事实，不是我们的计数。
...
- 观测随额度状态文件落盘（v2.9.3，用户要求），重启后「上游渠道」列不再清空。每条观测带 `at` 时间戳——持久化的是「X 时刻观察到的是哪家」这个事实，新不新由看的人拿时间戳判断；面板不把旧观测伪装成现在的结论。
...
| 文件 | 行数 | 职责 |
| --- | --- | --- |
| `index.js` | ~950 | 插件入口：装配各模块 + 实现 fetch 层拦截（Cline 特征头注入 + **上游观测** + 换 Key 轮换，轮换成功后选定跟随）+ 面板只读/导入/重置/选定四条路由 |
...
| `lib/host/quota-state.js` | ~400 | 状态落盘：额度冷却 + **用量统计 / 累计计数** + **按模型**的「使用中」选定（同一文件、同一去抖写入器）+ 诊断白名单 + 重试窗口解析（报文文本与 `Retry-After` 头） |
| `lib/host/request-shape.js` | ~100 | 读请求形状：body 可否重发、模型名、鉴权头读写 |
| `lib/host/settings-table.js` | ~53 | DSH 设置服务的兼容读取：旧版 `settings.get(ns)`（≤0.1.6）与新版 `SettingsForms.describe()`（0.1.7+）两代 API 通吃，读不到一律返回 `undefined` 绝不抛错 |
| `lib/host/upstream.js` | ~150 | 上游观测：从响应读出上游名——`provider_metadata.gateway.routing.finalProvider`（deepseek 形态）**或 chunk 顶层 `provider`**（glm 形态）+ 内存台账（model / key+model）+ 面板数据（含「是否漂离预期」） |
| `lib/host/pin.js` | ~85 | **上游锁定**：`pinUpstream` 配置解析 + 往请求体注入 `provider: { only, allow_fallbacks }`（本插件唯一会改写请求体的模块） |
| `lib/host/credentials.js` | ~87 | 兜底读取 `.credentials.yaml` 的 refs 段；凭据服务缺席时的兜底写入（保留其余内容 + 临时文件改名） |
| `lib/host/key-import.js` | ~113 | 面板导入：粘贴文本解析、空闲 ref 分配、写入编排（不含 Key 原文的返回值） |
| `lib/host/usage.js` | ~100 | Token 用量采集（tee 出只读分支扫 usage） |
| `lib/host/key-pool.js` | ~476 | key 池：来源、按 `key+模型` 冷却、粘性选 Key、**按模型**的「使用中」选定（含轮换跟随）、按模型用量（按 8 位标签跨重启恢复） |
| `lib/host/status.js` | ~168 | 设置面板的状态载荷（唯一的对外数据出口，字段白名单） |
| `lib/host/http.js` | ~107 | 路由的 JSON 响应（含 ETag / 304 条件请求）、同源校验、带上限的 JSON 正文读取 |
| `lib/client.js` | ~1310 | 浏览器半边：设置页分区（含导入按钮与弹窗、重置统计、按模型选定「使用中」的 Key）（**必须单文件**，见下） |
...
`cline-panel-check.mjs` 的 **PB 组**在真实渲染树里断言「上游只在 Key 表出现」：模型芯片上**不再挂上游徽标**（渠道来源不在那个位置重复出现），上游只出现在 Key 表的「上游渠道」列里、漂到非预期上游时带警示色、没观测到时显示「—」、英文界面表头走词典。**I 组**断言「速度」列：tok/s = 累计输出 ÷ 累计流式秒数、条为单段纯输入占比、没有耗时数据时显示「—」而不是 0、合计行给出整体加权速度、且不再出现命中率（v2.9.0 起）。**X/Y 组**钉住设置服务的两代 API（v2.9.8 起）：Y 组走 0.1.7+ 的 `describe()`（挂载即入池 + `settings/document-updated` 事件触发的 apiKeyEnv 热重挂 + 无关命名空间不搅乱池子），X 组锁住旧版 `get(ns)` 路径不死。
